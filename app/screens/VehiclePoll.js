...
function VehiclePoll({ children, style, navigation }) {
  ...
  const submit = async () => {
    ...
    AsyncStorage.setItem("completed", JSON.stringify({ ...store, vehiclePoll: true }))
      .then(() => navigation.navigate("details"))
      .catch((err) => console.log(err));
    AsyncStorage.setItem("vehiclePoll_url", JSON.stringify({ imageUri }));
  };
